...
> # ⚠️ SUPERSEDED
>
> **[`/CLAUDE.md`](../CLAUDE.md) — the Mevora Multi-Agent Development Policy — is now the
> authoritative multi-agent policy.** Where this document disagrees with it, **CLAUDE.md wins**.
>
> Three instructions in this document are explicitly superseded:
>
> | This document (superseded) | Current standard |
> |---|---|
> | `agent/*` branch prefix | semantic prefixes: `fix/` `feat/` `audit/` `perf/` `chore/` `docs/` `test/` — no mandatory `agent/*` |
> | worktree root `D:\Mevora-agents\<slug>` | `D:\Mevora-worktrees\<task-name>` for all **new** worktrees |
> | base branch `main` (local) | `origin/main`, verified after `git fetch origin` |
>
> Existing worktrees created under the older layouts are **grandfathered** — they are not
> moved or renamed by the new policy; cleanup is a separate future chore.
>
> The rest of this document is retained unchanged for historical context.
