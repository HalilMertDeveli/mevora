...
export async function cleanupStalePendingUploads(
  ...
  for (const file of files) {
    ...
    if (!userExists) {
      // Handled by remnant cleanup; still safe to remove pending.
    }
...
export async function cleanupOrphanChatMedia(
  ...
  for (const file of files) {
    ...
    if (userExists && matchSnap.exists && matchSnap.data()?.isActive === true) {
      // Active chat — do not delete without message-path proof of orphan.
      ...
      // Message missing but match active — may be in-flight upload; skip unless very old.
...
export async function cleanupOldAuditLogs(
  ...
  const result = emptyResult(options.dryRun);
  // Default retain 180 days; mass delete of audit requires admin approval flag.
...
export async function refreshDiscoverEligibilityBatch(
  ...
  for (const doc of snap.docs) {
    ...
    result.candidates.push({uid: doc.id, reason: missing ? "missing_user" : "banned_or_deleted"});
    if (!options.dryRun) {
      await doc.ref.set(
        {
      ...
      result.deleted += 1; // "deleted" = corrected
    }
  }
  return result;
}
